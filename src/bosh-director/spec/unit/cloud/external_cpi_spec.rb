  ...
  def self.it_calls_cpi_method(method, *arguments)
    ...
    context 'api version specified' do
      ...
      it 'should call cpi binary with correct arguments' do
        ...
        expected_stdin = %({"method":"#{method}","arguments":#{arguments.to_json},"context":{"director_uuid":\
                         "fake-director-uuid","request_id":"cpi-fake-request-id"},"api_version":1})
    ...
    context 'api version not specified' do
      it 'should call cpi binary with correct arguments' do
        ...
        expected_cmd = '/path/to/fake-cpi/bin/cpi'
        expected_stdin = %({"method":"#{method}","arguments":#{arguments.to_json},"context":{"director_uuid":\
                         "fake-director-uuid","request_id":"cpi-fake-request-id"}})
    ...
    describe 'error response' do
      def self.it_raises_an_error(error_class, error_type = error_class.name, message = 'fake-error-message')
        ...
        it 'raises an error constructed from error response' do
          expect { call_cpi_method }.to raise_error(error_class, /CPI error '#{error_type}' with message '#{message}' \
                   in '#{method}' CPI method \(CPI request ID: 'cpi-fake-request-id'\)/)
        end
